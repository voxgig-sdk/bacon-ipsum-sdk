...
class BaconIpsumTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            BaconIpsumTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = BaconIpsumTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = BaconIpsumTestRunner.getenv("BACON_IPSUM_TEST_LIVE")
        override = BaconIpsumTestRunner.getenv("BACON_IPSUM_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = BaconIpsumTestRunner.getenv(key)
        ...
        explain = BaconIpsumTestRunner.getenv("BACON_IPSUM_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["BACON_IPSUM_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if BaconIpsumTestRunner._test_control is not None:
            return BaconIpsumTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                BaconIpsumTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            BaconIpsumTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = BaconIpsumTestRunner.load_test_control()
...
def load_env_local():
    BaconIpsumTestRunner.load_env_local()


def env_override(m):
    return BaconIpsumTestRunner.env_override(m)


def entity_data(v):
    return BaconIpsumTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return BaconIpsumTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return BaconIpsumTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return BaconIpsumTestRunner.load_test_control()


def live_client_options():
    return BaconIpsumTestRunner.live_client_options()


def live_delay_ms():
    return BaconIpsumTestRunner.live_delay_ms()
