    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BACON_IPSUM_TEST_TEXT_GENERATION_ENTID JSON to run live")
        client = setup["client"]
        ...
        text_generation_ref01_data_dt0_loaded = text_generation_ref01_ent.load(text_generation_ref01_match_dt0, None)
...
def _text_generation_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("BACON_IPSUM_TEST_TEXT_GENERATION_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("BACON_IPSUM_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("BACON_IPSUM_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("BACON_IPSUM_TEST_EXPLAIN") == "TRUE",
        "live": _live,
