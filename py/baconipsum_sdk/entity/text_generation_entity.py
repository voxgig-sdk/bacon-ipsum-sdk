...
from baconipsum_sdk.baconipsum_types import (
    TextGeneration,
...
class TextGenerationEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return TextGenerationEntity(self._client, opts)
    ...
    def data_get(self) -> TextGeneration:
    ...
    def match_get(self) -> TextGeneration:
    ...
    def load(self, reqmatch=None, ctrl=None) -> TextGeneration:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.TextGeneration().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
