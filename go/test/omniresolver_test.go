...
import (
	...
	omni "github.com/voxgig-sdk/bacon-ipsum-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.BaconIpsumError); is {
				out := map[string]any{
					"name":    "BaconIpsumError",
...
func omniCtx(arg any, client *sdk.BaconIpsumSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.BaconIpsumSDK); is {
			client = live
