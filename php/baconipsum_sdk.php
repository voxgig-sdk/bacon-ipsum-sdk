...
class BaconIpsumSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = BaconIpsumHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = BaconIpsumHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!BaconIpsumFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, BaconIpsumFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return BaconIpsumUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = BaconIpsumHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = BaconIpsumHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = BaconIpsumHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new BaconIpsumError($op . "_allow",
                "BaconIpsumSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = BaconIpsumHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new BaconIpsumError("graphql_error",
                "BaconIpsumSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function TextGeneration($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_text_generation === null) {
                $this->_text_generation = new TextGenerationEntity($this, null);
            }
            return $this->_text_generation;
        }
        return new TextGenerationEntity($this, $data);
    }
