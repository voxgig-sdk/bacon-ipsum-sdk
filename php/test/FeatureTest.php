...
class FtCtrl extends BaconIpsumControl
{
...
class FtClient
{
    ...
    private BaconIpsumUtility $utility;

    public function __construct(BaconIpsumUtility $utility, array $options)
    {
    ...
    public function get_utility(): BaconIpsumUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (BaconIpsumContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public BaconIpsumContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (BaconIpsumContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = BaconIpsumConfig::make_feature($name);
    ...
    private static function buildUrl(BaconIpsumSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof BaconIpsumError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new BaconIpsumResponse($fetched);
        }
    ...
    private function fail(BaconIpsumContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (BaconIpsumContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
