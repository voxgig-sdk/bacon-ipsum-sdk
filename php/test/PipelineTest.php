...
{
    private static function utility(): BaconIpsumUtility
    {
        return new BaconIpsumUtility();
    }
    ...
    private static function ctx(array $over = []): BaconIpsumContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new BaconIpsumContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new BaconIpsumOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof BaconIpsumError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new BaconIpsumOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = BaconIpsumMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new BaconIpsumOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = BaconIpsumMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new BaconIpsumOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = BaconIpsumMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new BaconIpsumSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = BaconIpsumMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new BaconIpsumResponse([]),
            'result' => new BaconIpsumResult([]),
        ]);
        [, $err] = BaconIpsumMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new BaconIpsumSpec([]),
            'response' => null,
            'result' => new BaconIpsumResult([]),
        ]);
        [, $err] = BaconIpsumMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new BaconIpsumSpec(['step' => 's']),
            'response' => new BaconIpsumResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new BaconIpsumResult([]),
        ]);
        [, $err] = BaconIpsumMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new BaconIpsumSpec(['step' => 's']),
            'response' => new BaconIpsumResponse(self::resp(200, ['v' => 1])),
            'result' => new BaconIpsumResult([]),
        ]);
        [, $err] = BaconIpsumMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new BaconIpsumSpec(['step' => 's']),
            'response' => new BaconIpsumResponse(self::resp(200, ['v' => 2])),
            'result' => new BaconIpsumResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        BaconIpsumMakeResponse::call($ctx);
    ...
    {
        $preset = new BaconIpsumResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new BaconIpsumSpec([]),
            'response' => new BaconIpsumResponse([]),
            'result' => new BaconIpsumResult([]),
        ...
        [$got, $err] = BaconIpsumMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new BaconIpsumResult([])]);
        [, $err] = BaconIpsumMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new BaconIpsumSpec([]), 'result' => null]);
        [, $err] = BaconIpsumMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new BaconIpsumSpec(['step' => 's']),
            'result' => new BaconIpsumResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new BaconIpsumOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = BaconIpsumMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new BaconIpsumSpec(['step' => 's']),
            'result' => new BaconIpsumResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new BaconIpsumOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = BaconIpsumMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new BaconIpsumResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new BaconIpsumSpec([]),
            'result' => new BaconIpsumResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = BaconIpsumMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = BaconIpsumMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new BaconIpsumError('boom', 'boom');
        $utility->fetcher = function (BaconIpsumContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new BaconIpsumSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = BaconIpsumMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (BaconIpsumContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = BaconIpsumMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(BaconIpsumResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        BaconIpsumMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (BaconIpsumContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new BaconIpsumSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = BaconIpsumMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new BaconIpsumResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new BaconIpsumSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = BaconIpsumMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = BaconIpsumMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = BaconIpsumMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new BaconIpsumResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, BaconIpsumDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new BaconIpsumResult(['ok' => false])]);
        $this->expectException(BaconIpsumError::class);
        BaconIpsumDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new BaconIpsumResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', BaconIpsumMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        BaconIpsumMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new BaconIpsumBaseFeature();
        $b = new BaconIpsumBaseFeature();
        BaconIpsumFeatureAdd::call($ctx, $a);
        BaconIpsumFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): BaconIpsumBaseFeature
    {
        $f = new BaconIpsumBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        BaconIpsumFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): BaconIpsumContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new BaconIpsumSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = BaconIpsumPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        BaconIpsumPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new BaconIpsumResponse(['headers' => null]),
            'result' => new BaconIpsumResult([]),
        ]);
        BaconIpsumResultHeaders::call($ctx);
