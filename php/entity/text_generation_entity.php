...
// BaconIpsum SDK TextGeneration entity
...
class TextGenerationEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new TextGenerationEntity($this->_client, $opts);
    }

    /**
     * @param TextGeneration|array $args TextGeneration data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = BaconIpsumHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return TextGeneration|array The current TextGeneration data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of TextGeneration fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = BaconIpsumHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of TextGeneration fields).
     */
    ...
    /**
     * Load a single TextGeneration.
     *
     * @param TextGenerationLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed TextGenerationLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return TextGeneration|array The loaded TextGeneration as an assoc-array at the
     *   SDK boundary; throws BaconIpsumError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = BaconIpsumHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
