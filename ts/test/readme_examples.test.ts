...
//     (\`new BaconIpsumSDK(...)\` / \`BaconIpsumSDK.test(...)\` become
//     \`BaconIpsumSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"text_generation":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new BaconIpsumSDK(...)\` and any
// \`BaconIpsumSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+BaconIpsumSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/BaconIpsumSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
