...
module BaconIpsumUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = BaconIpsumResponse.new({})
    ...
    if fetch_err
      ...
      response = BaconIpsumResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = BaconIpsumResponse.new(fetched)
    else
