// Command BaconIpsum-cli is a CLI + REPL for the BaconIpsum SDK,
...
const prompt = "bacon-ipsum"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "text_generation"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      BACON_IPSUM_APIKEY (api key), BACON_IPSUM_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: BACON_IPSUM_APIKEY carries the API key and
	// BACON_IPSUM_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("BACON_IPSUM_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("BACON_IPSUM_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewBaconIpsumSDK(opts)
