...
boru-driven command-line client **and** interactive REPL for the BaconIpsum
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/bacon-ipsum-cli)
make build

# 2. See usage (words, entities, env vars)
./bacon-ipsum-cli --help

# 3. Provide credentials once, via the environment
export BACON_IPSUM_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./bacon-ipsum-cli load 1 text_generation            # {id:1} shorthand
./bacon-ipsum-cli load '{id:1}' text_generation       # explicit match map

# 5. Override the API base URL for a single call
BACON_IPSUM_BASE=https://api.example.com ./bacon-ipsum-cli load 1 text_generation

# 6. No arguments -> interactive REPL
./bacon-ipsum-cli
bacon-ipsum> load 1 text_generation
bacon-ipsum> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/bacon-ipsum-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/bacon-ipsum-cli load 1 text_generation
   ```
...
```sh
./bacon-ipsum-cli load 1 text_generation          # scalar shorthand for {id:1}
./bacon-ipsum-cli load '{id:1}' text_generation     # explicit match map
```
...
./bacon-ipsum-cli load 1 text_generation
```
...
Run with no arguments to open a REPL (prompt `bacon-ipsum>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `text_generation`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/bacon-ipsum-cli`. |
...
(`./bacon-ipsum-cli <expr>`) and interactively (the REPL), and expressions compose the
