...
require_relative 'BaconIpsum_types'
...
class BaconIpsumSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = BaconIpsumHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = BaconIpsumHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, BaconIpsumFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    BaconIpsumUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = BaconIpsumHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = BaconIpsumHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = BaconIpsumHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = BaconIpsumHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = BaconIpsumError.new(
        "graphql_error", "BaconIpsumSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
