...
class TextGenerationEntity
  ...
  def make
    opts = @_entopts.dup
    TextGenerationEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = BaconIpsumHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [TextGeneration, Hash] the current TextGeneration data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = BaconIpsumHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of TextGeneration fields)
  def match_get
  ...
  #
  # @param reqmatch [TextGenerationLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.TextGeneration.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [TextGeneration, Hash] the loaded TextGeneration; raises BaconIpsumError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = BaconIpsumHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
